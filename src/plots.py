...
def create_heatmap_visualizations(results_df, results_dir):
    ...
    for metric in metrics:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, f"heatmap_token_wise_{metric}.png"))
        plt.close()
...
def create_execution_time_plot(results_df, results_dir):
    ...
    grouped_time = results_df.groupby(['chunk_size', 'num_retrieved_chunks'])['execution_time'].mean().reset_index()

    for chunk_size in results_df['chunk_size'].unique():
        subset = grouped_time[grouped_time['chunk_size'] == chunk_size]
        plt.plot(subset['num_retrieved_chunks'], subset['execution_time'],
